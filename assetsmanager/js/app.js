// Source from: https://github.com/Pageclip/valid-form/blob/master/dist/valid-form.min.js
// @TODO: Import it dynamically from node_modules package.
...
var _page = function(window) {

    /**
    * Solution based on HTML5 validation using field checkValidity
    * Supports custom field messages
    * Not relying upon form reportValidity (due poor browser support)
    * Depends on ValidForm module
    **/

    // Set validation listeners
    function setFieldValidations (form) {
         ValidForm(form, {
    ...
    // Triggers validity events based on selection of fields.
    ...
    function initFormValidation(form) {
        if (!form) {
            throw Error ('Form element required with novalidate attribute');
        }

        // Trigger validation checks on form submit
        // Important: requires novalidate attribute on form
    ...
    // Export
    window.checkInputValidity = checkInputValidity;
};

document.addEventListener('DOMContentLoaded', _page.bind(window, window));
